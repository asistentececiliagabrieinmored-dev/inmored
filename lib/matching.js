  ...
  let queryReferencias = client
    ...
    .select(
      'id, ubicacion, precio, moneda, dimensiones, dormitorios, contacto_nombre, contacto_telefono, ' +
        'descripcion, zona_id, tipo_transaccion:tipos_transaccion(nombre)'
    )
    .eq('activa', true)
...
function formatearDetalleCoincidencia({ titulo, contactoTexto, transaccionTexto, precioTexto, ubicacion, dimensiones, dormitorios, banos }) {
  ...
  referencias.forEach((r) => {
    lineas.push(
      formatearDetalleCoincidencia({
        ...
        contactoTexto: r.contacto_nombre
