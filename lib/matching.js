...
export async function buscarCoincidenciasParaRequerimiento(client, criterios) {
  ...
  // Un inmueble sin zona asignada no se descarta: queda pendiente de que lo
  // evalúe el filtro de ubicación (texto/IA) en vez de perderse por un dato
  // que nunca se terminó de cargar.
...
export function formatearResumenCoincidencias(clienteNombre, inmuebles, referencias) {
  const total = inmuebles.length + referencias.length;

  const lineas = [
    total > 0
      ? `🔔 Encontré ${total} coincidencia(s) para "${clienteNombre}":`
      : `Requerimiento guardado para "${clienteNombre}". Todavía no hay coincidencias, te avisamos apenas aparezca algo.`,
  ];
