...
export async function buscarCoincidenciasParaRequerimiento(client, criterios) {
  ...
  if (ubicacionReferencia) queryInmuebles = queryInmuebles.ilike('ubicacion', `%${ubicacionReferencia}%`);
  ...
  if (ubicacionReferencia) queryReferencias = queryReferencias.ilike('ubicacion', `%${ubicacionReferencia}%`);
...
export async function buscarRequerimientosCoincidentes(client, criterios) {
  ...
  return candidatos.filter((req) => {
    ...
    if (req.ubicacion_referencia) {
      if (!ubicacion || !ubicacion.toLowerCase().includes(req.ubicacion_referencia.toLowerCase())) return false;
    }
