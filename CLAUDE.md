...
- **Solución de fondo al problema de tildes/mayúsculas/abreviaturas en `procesarReferencia`**: se eliminó `encontrarCoincidencia` (texto libre de Claude + matching manual por string, frágil) y se reemplazó por el mismo patrón que ya usaba `/requerimiento`: Claude elige directamente `tipoInmuebleId`/`tipoTransaccionId`/`zonaId` del catálogo real vía `enum` en el schema estructurado. Como el modelo entiende el significado (no compara texto literal), el problema de "anticrético" vs "anticretico" desaparece de raíz en vez de parchearse. `construirEsquemaReferencia` ahora recibe los catálogos igual que `construirEsquemaRequerimiento`.
- **El filtro no descarta candidatos con tipo de inmueble/transacción sin resolver** (mismo problema que la zona, ahora también en `tipo_inmueble_id`/`tipo_transaccion_id`). Se detectó con un caso real: una referencia de "casa en anticrético en Sevilla" tenía `tipo_transaccion_id: null` (nunca se resolvió al cargarla), y quedaba excluida de cualquier búsqueda de anticréticos aunque la descripción lo dijera textualmente.
- **El filtro de zona no descarta candidatos sin zona asignada** (`zona_id is null` se deja pasar, no solo `zona_id in (...)`) — muchas referencias externas no tienen zona resuelta, y antes quedaban excluidas aunque la ubicación coincidiera perfectamente. Se detectó con un caso real: un requerimiento de "terreno en alquiler, avenida Doble Vía a La Guardia" no encontraba nada porque Claude le había asignado una zona (aunque el cliente solo dio una avenida) y las referencias candidatas tenían `zona_id: null`. También se ajustó el prompt del comando `/requerimiento` para que solo asigne zona cuando el cliente la nombra explícitamente, en vez de intentar adivinarla a partir de una dirección puntual.
- **Comando `/requerimiento <texto libre>`**: permite cargar un requerimiento directo desde Telegram (sin pasar por la web). Claude Haiku extrae los datos y, a diferencia de las referencias, elige directamente los IDs de tipo/transacción/zona de los catálogos reales (no hace falta un segundo paso de "encontrar coincidencia" por texto) — para zonas, se le pide explícitamente que elija la más parecida si el cliente no la nombra exactamente igual. Corre el matching al instante y responde con las coincidencias, igual que el formulario web (`lib/matching.js` → `formatearResumenCoincidencias`, compartido entre ambos flujos).
- **`requerimientos.cliente_nombre` → `nombre_requerimiento`, se sacó `cliente_telefono` de la web y del bot.** Ningún asesor (sobre todo de otra inmobiliaria, pensando en el uso multi-agencia a futuro) va a querer guardar el nombre y teléfono real de sus clientes en un sistema compartido — da la sensación de que le quieren captar el contacto. Ahora es una referencia libre que el asesor elige (no hace falta que sea el nombre real). En el bot, el comando `/requerimiento` usa un formato de dos líneas: la primera es el nombre del requerimiento (tal cual, sin pasar por IA), el resto es lo que Claude extrae. Ver `supabase/requerimientos_nombre_requerimiento.sql` — **pendiente de correr**. La columna `cliente_telefono` se dejó en la tabla (sin uso) para no romper nada; se puede eliminar más adelante si nunca se usa.
