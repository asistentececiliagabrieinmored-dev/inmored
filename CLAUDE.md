...
- **`encontrarCoincidencia` (matching de texto contra catálogos: tipo de inmueble, tipo de transacción, zona) ahora ignora acentos**, no solo mayúsculas. Probablemente la causa raíz del caso anterior: Claude extrae "anticrético" (con tilde, correcto en español) pero el catálogo tiene `"anticretico"` sin tilde — la comparación fallaba silenciosamente y el campo quedaba en null. Reutiliza `normalizarParaComparar` de `lib/matching.js` (mismo mecanismo que ya usa el filtro de ubicación).
