...
- **Pendiente**: generar códigos de activación para el resto de los asesores (ver ejemplo de INSERT en `supabase/telegram_bot_activacion.sql`), construir la lógica de auto-matching entre `referencias_externas` y `requerimientos`, y la webapp pública multi-agencia (fuera de alcance por ahora).
